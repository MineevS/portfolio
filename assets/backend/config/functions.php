        ...
        foreach ($array_data as $data) {
            $html = $html .
                '<form method="POST" action="/assets/frontend/pages/project.php" style=" width: 100%; height: fit-content;">
                        <button class="MainProjectBody" type="submit" style="display: flex; border: none; background-color: #F6F6F6; width: 100%; height: 100%;">
                            <div class="div-left" style="display: flex; justify-content: center; width: 10%; height: 100%; " > <!-- background-color: red;-->
                                <p class="projectIndexTitle" style="align-self: start;">01<p>
                            </div>
                            <div class="div-right" style="width: 100%; padding: 0 0 0 2%; display:  flex; flex-direction: column; text-align: left; height: 100%;  border-left: 2px solid; border-color: black;"> <!-- background-color: green; -->
                                <h1 class="projectTitle">Название проекта</h1>
                                <img class="projectImg"/>
                                <p class="projectBody" style="align-self: flex-end;">Описание проекта</p>
                                <p class="projectStatus">Запуск</p>
                                <div class="projectLikeComms" style="display: flex; align-items: center; gap: 10px; height: fit-content;">
    ...
    if ($status) {
        ...
        foreach ($array_data as $data) {
            switch ($params["for"]) {
                case 'properties':
                    $html =  $html . '
                        <string class="stringProfile">Группа*:                    <input class="inputProfile" id="group"                value="' . $data['group'] . '"                  readonly /></string>                                       
                        <string class="stringProfile">Курс*:                      <input class="inputProfile" id="course"               value="' . $data['course'] . '"                 readonly /></string>                         
                        <string class="stringProfile">Шифр*:                      <input class="inputProfile" id="cipher"               value="' . $data['cipher'] . '"                 readonly /></string>
                        <string class="stringProfile">Навыки:                    <input class="inputProfile" id="skills"               value="' . $data['skills'] . '"                 readonly /></string>  
                        <string class="stringProfile">Институт*:                  <input class="inputProfile" id="institute"            value="' . $data['institute'] . '"              readonly /></string>
                        <string class="stringProfile">Год приёма*:                <input class="inputProfile" id="year_start"           value="' . $data['year_start'] . '"             readonly /></string>   
                        <string class="stringProfile">Специальность*:             <input class="inputProfile" id="specialization"       value="' . $data['specialization'] . '"         readonly /></string>                <!-- (Направление) -->
                        <string class="stringProfile">Образовательная программа*: <input class="inputProfile" id="educational_program"  value="' . $data['educational_program'] . '"    readonly /></string>';
                    break;
                case 'about':
                    $html = $html . '<input class="inputProfile" id="about" value="' . $data['about'] . '" readonly />';
                    break;
            }
        }
    }
...
function psql_query_properties_project($params, $smarty)
{
    ...
    $html =  $html . '
            <string class="stringProfile">Дата выхода:                    <input id="date-preview"            value=""    readonly /></string>                                       
            <string class="stringProfile">Статус:                         <input id="status"                  value=""    readonly /></string>                         
            <string class="stringProfile">Стек:                           <input id="stack"                   value=""    readonly /></string>
            <string class="stringProfile">Оценка сообщества:              <input id="scores_communities"      value=""    readonly /></string>  
            <string class="stringProfile">Оценка знатаков:                <input id="scores_experts"          value=""    readonly /></string>
            <string class="stringProfile">Популярные теги проекта:        <input id="populars_tags"           value=""    readonly /></string>';
...
function psql_query_article($params, $smarty)
{
    ...
    if (isset($params['head1']) && isset($params['head2']) && isset($params['head3']))
        $html .= '<article style="display: grid; width: 80%; align-self: flex-start;">
            <h1 class="HelveticaMain" style="justify-self: start;">' . $params['head1'] . '</h1>
            <p class="VasekMain" style="justify-self: center;">' . $params['head2'] . '</p>
            <h1 class="HelveticaMain" style="justify-self: end;">' . $params['head3'] . '</h1>
        </article>';
    return $html;
}
